...
var SelectedOptionContext = UpdatingContext.extend({
	...
	render: function() {
		...
		//TODO somehow push the value? this.pushValue()
		
		return this.bind() ? '<script ' + name + '="' + id + '" type="text/x-placeholder"></script>' : '';
	},
	...
	pushValue: function(value) {
		var selected, model = _.isSubscribable(this._model) ? this._model() : this._model, //TODO extract subscribable helper
			...
			isMultiple = element.parent('select').prop('multiple'),
			optionElements = element.sibilings('option'); 
